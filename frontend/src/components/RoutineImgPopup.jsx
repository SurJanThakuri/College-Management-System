...
const RoutineImgPopup = ({ data, onClose }) => {
    const { _id } = data;
    ...
    const onSubmit = (data) => {
        ...
        axios.patch(`${API_URL}/admin/routines/update-routine-image/` + _id, formData, {
            headers: {
        ...
        .then(response => {
            navigate('/admin-dashboard/routines/' + _id);
        })
        .catch(error => {
            console.error('Failed to update routine image:', error);
        });
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white w-full p-4 relative rounded-xl max-w-sm max-h-[80vh] sm:max-w-md sm:max-h-[60vh] md:max-w-sm md:max-h-[60vh] lg:max-w-[40vw]">
                <div className="absolute top-0 left-0 right-0 flex justify-between items-center p-4 sm:p-6">
                    <p className="font-bold text-xl">Routine Image</p>
                    <button onClick={onClose}>
                        <img className='w-4' src="/images/close.png" alt="" />
                    </button>
                </div>
                <div className="flex flex-col items-center mt-6">
                    <img src={data.routineImage} alt="Profile" className=" w-32 h-24 m-4 sm:w-40 sm:h-32 object-contain" />
                </div>
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center mt-6">
                    <div className="flex items-center w-full">
                        <InputField
                            ...
                            {...register("routineImage", { required: "Routine Image is required" })}
                            className="flex-grow"
                        />
                    </div>
...
export default RoutineImgPopup;
